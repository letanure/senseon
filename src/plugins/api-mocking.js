...
export default function (context) {
  ...
  const worker = setupWorker(
    ...
    rest.get(`${API_PATH}/counter/reset`, (req, res, ctx) => {
      window.sessionStorage.setItem(COUNTER_VALUE_KEY, '0')
      return res(
        ctx.json({
          value: 0,
    ...
    rest.get(`${API_PATH}/counter`, (req, res, ctx) => {
    ...
    rest.get(`${API_PATH}/counter/decrement`, (req, res, ctx) => {
      const oldValue = parseInt(
        window.sessionStorage.getItem(COUNTER_VALUE_KEY)
      )
    ...
    rest.get(`${API_PATH}/counter/increment`, (req, res, ctx) => {
      const oldValue = parseInt(
        window.sessionStorage.getItem(COUNTER_VALUE_KEY)
      )
      const newValue = oldValue + 1
    ...
    rest.get(`${API_PATH}/data`, (req, res, ctx) => {
      const data = []
      for (let i = 0; i < 10; i++) {
        data.push({
          label: `Slice ${i + 1}`,
    ...
    rest.post(`${API_PATH}/login`, (req, res, ctx) => {
      ...
      if (
        !Object.keys(USER_CREDENTIALS).includes(username) ||
